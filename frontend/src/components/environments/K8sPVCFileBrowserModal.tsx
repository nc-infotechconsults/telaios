...
import {
  listK8sPVCFiles,
  downloadK8sPVCFile,
  getK8sPVCFileContent,
  updateK8sPVCFileContent,
} from "../../lib/api";
import { toast } from "../../lib/toast";
import type { K8sPVCFileEntry, K8sPVCFileContent } from "../../types";
...
export default function K8sPVCFileBrowserModal({
  environmentId,
  namespace,
  pvcName,
  isOpen,
  ...
  const [entries, setEntries] = useState<K8sPVCFileEntry[]>([]);
  const [browserLoading, setBrowserLoading] = useState(false);
  const [browserError, setBrowserError] = useState<string | null>(null);
  const [conflictPod, setConflictPod] = useState<string | null>(null);
  const [downloading, setDownloading] = useState<string | null>(null);

  // ─ Editor state
  const [fileData, setFileData] = useState<K8sPVCFileContent | null>(null);
  ...
  const loadDir = useCallback(
    async (path: string) => {
      ...
      setConflictPod(null);
      try {
        const files = await listK8sPVCFiles(environmentId, pvcName, namespace, path);
        setEntries(files);
        setCurrentPath(path);
      } catch (err: unknown) {
        // Check for 409 conflict
        const axiosErr = err as { response?: { status?: number; data?: { conflicting_pod?: string; error?: string } } };
        if (axiosErr?.response?.status === 409) {
          const pod = axiosErr.response.data?.conflicting_pod ?? "unknown";
          setConflictPod(pod);
          setEntries([]);
        } else {
          setBrowserError("Failed to list files. The PVC may be empty or inaccessible.");
          setEntries([]);
        }
      } finally {
    ...
    [environmentId, pvcName, namespace],
  );
  ...
  const handleDownload = async (entry: K8sPVCFileEntry) => {
    setDownloading(entry.path);
    try {
      await downloadK8sPVCFile(
        environmentId,
        pvcName,
        namespace,
        entry.path,
  ...
  const handleOpenFile = async (entry: K8sPVCFileEntry, mode: "view" | "edit") => {
    setLoadingFile(true);
    setOpeningFilePath(entry.path);
    try {
      const data = await getK8sPVCFileContent(environmentId, pvcName, namespace, entry.path);
      setFileData(data);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateK8sPVCFileContent(environmentId, pvcName, namespace, fileData.path, editorContent);
        ...
        {(onClose) => (
          <>
            <ModalHeader className="flex flex-col gap-0 pb-2">
              <span className="text-sm font-semibold font-mono">{pvcName}</span>
              {panelView === "browser" ? (
                <span className="text-xs text-default-400 font-normal">
                  File Browser · <span className="font-mono">{namespace}</span>
                </span>
              ) : (
            ...
            <ModalBody>
              {panelView === "browser" ? (
                <div className="flex flex-col gap-0 px-4 py-2">
                  ...
                  {conflictPod && (
                    <div className="flex items-start gap-2 rounded-lg bg-danger-50 border border-danger-200 px-4 py-3 mb-2 text-danger-700">
                      <span className="text-sm flex-1">
                        This PVC is currently mounted by pod{" "}
                        <span className="font-mono font-semibold">{conflictPod}</span>.
                        Unmount it first to browse the files.
                      </span>
                    </div>
                  )}
