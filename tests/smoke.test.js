 ...
 * Usage:
 *   npm test
 *   SERVER_URL=http://localhost:8000 npm test
 *
 * Auth: all server routes (except /health and /auth) require authentication.
 * Registers/logs in a smoke-test user for user-facing routes. INTERNAL_API_KEY
 * is used only for internal service routes and first-run admin settings calls.
 */
...
async function authenticateSmokeUser() {
  ...
  try {
    ...
    const { data } = await axios.post(`${SERVER_URL}/auth/login`, { email, password });
    token = data.token;
  }
  smokeToken = token;
...
async function runApiTests() {
  ...
  await test("GET /projects returns list with new project", async () => {
    ...
    assert(data.items.some((p) => p.id === projectId), "contains created project");
  });
  ...
  await test("PATCH /projects/:id/repositories/:id updates repo status", async () => {
    const { data } = await api.patch(`/projects/${projectId}/repositories/${repoId}`, {
      status: "ready",
    });
  ...
  await test("POST /plans creates plan", async () => {
    const { data, status } = await api.post(`/projects/${projectId}/plans`, {});
    assert(status === 201, `Expected 201, got ${status}`);
    assert(data.id, "has id");
  ...
  await test("GET /plans?project_id=... returns plan", async () => {
    const { data } = await api.get(`/projects/${projectId}/plans`);
    assert(Array.isArray(data), "is array");
  ...
  await test("POST /tasks creates task", async () => {
    const { data, status } = await api.post(`/plans/${planId}/tasks`, {
  ...
  await test("POST /tasks creates dependent task", async () => {
    const { data } = await api.post(`/plans/${planId}/tasks`, {
  ...
  await test("GET /plans/:id/tasks returns tasks with deps", async () => {
    const { data } = await api.get(`/plans/${planId}/tasks`);
    assert(Array.isArray(data), "is array");
  ...
  await test("GET /plans/:id returns plan", async () => {
    const { data } = await api.get(`/plans/${planId}`);
    assertEqual(data.id, planId, "plan id");
  });

  // Messages
  await test("POST /messages creates message", async () => {
    const { data, status } = await api.post(`/projects/${projectId}/messages`, {
      role: "user",
  ...
  await test("GET /projects/:id/messages returns messages", async () => {
...
//
// Simulates what the execution runtime does using the
...
async function runExecutionLifecycleTests() {
  ...
  try {
    ...
    await test("Lifecycle/setup: create plan with 3 linear tasks", async () => {
      const { data: plan } = await api.post(`/projects/${projectId}/plans`, {});
      planId = plan.id;

      const makeTask = (title, order, deps) =>
        api.post(`/plans/${planId}/tasks`, {
          title,
          description: title,
          type: "code",
    ...
    await test("Lifecycle/happy: PATCH internal/plans/:id/status → executing", async () => {
      const { data } = await internalApi.patch(`/internal/plans/${planId}/status`, {
        status: "executing",
      });
    ...
    await test("Lifecycle/happy: create task artifacts for T1", async () => {
      const { status } = await internalApi.post(`/internal/tasks/${t1Id}/artifacts/bulk`, {
        artifacts: [
          {
            artifact_type: "diff",
    ...
    await test("Lifecycle/happy: GET /plans/:id shows completed with all done tasks", async () => {
      ...
      const { data: tasks } = await api.get(`/plans/${planId}/tasks`);
      const allDone = tasks.every((t) => t.status === "done");
      assert(allDone, `all tasks done — got: ${tasks.map((t) => t.status).join(", ")}`);
    });
    ...
    await test("Lifecycle/failure: confirm plan and start executing", async () => {
      ...
      const { data } = await internalApi.patch(`/internal/plans/${planB}/status`, { status: "executing" });
    ...
    await test("Lifecycle/failure: POST skip-dependents → T2 and T3 become skipped", async () => {
      const { data } = await internalApi.post(`/internal/tasks/${bT1}/skip-dependents`);
      assert(typeof data.skipped === "number", "skipped count returned");
      assert(data.skipped >= 2, `Expected ≥2 skipped, got ${data.skipped}`);

      const { data: tasks } = await api.get(`/plans/${planB}/tasks`);
    ...
    await test("Lifecycle/failure: PATCH plan → failed with failure_reason", async () => {
      const { data } = await internalApi.patch(`/internal/plans/${planB}/status`, {
        status: "failed",
    ...
    await test("Lifecycle/failure: GET /plans/:id reflects final failed state", async () => {
      ...
      const { data: tasks } = await api.get(`/plans/${planB}/tasks`);
      const t1Task = tasks.find((t) => t.id === bT1);
    ...
    await test("Lifecycle/cancel: confirm plan and start executing", async () => {
      ...
      await internalApi.patch(`/internal/plans/${planC}/status`, { status: "executing" });
    });

    await test("Lifecycle/cancel: POST cancel-tasks cancels pending tasks", async () => {
      const { data } = await internalApi.post(`/internal/plans/${planC}/cancel-tasks`);
      ...
      const { data: tasks } = await api.get(`/plans/${planC}/tasks`);
...
async function runServerHealthTests() {
  console.log("\n── Server Health ─────────────────────────────────────────────");
  ...
  try {
    ...
    await test("SSE connects to /chat/:planId/stream with correct headers", () =>
      new Promise((resolve, reject) => {
        ...
        const agentUrl = new URL(`/chat/${planId}/stream`, SERVER_URL);
        const transport = agentUrl.protocol === "https:" ? https : http;

        const req = transport.request(
          agentUrl,
          { method: "GET", headers: { Authorization: `Bearer ${smokeToken}` } },
          (res) => {
...
async function main() {
  ...
  try {
    ...
    console.error("Fatal error in API tests:", err.message);
  }
  ...
  try {
    await runServerHealthTests();
  } catch (err) {
    console.error("Fatal error in server health tests:", err.message);
  }
