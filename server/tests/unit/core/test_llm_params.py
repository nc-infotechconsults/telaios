...
from telaios.core.llm import build_chat_model as build_llm
...
@pytest.mark.skipif(
    not _langchain_openai_installed,
    reason="Phase 6: langchain_openai not installed",
)
class TestBuildChatModelParams:
    def test_openai_no_extra_params(self):
        with patch("langchain_openai.ChatOpenAI") as mock_cls:
            ...
            build_llm(
                _build_llm_config(
                    {"llm_provider": "openai", "llm_model": "gpt-4o", "llm_api_key_raw": "sk-test"}
                )
            )
        call_kwargs = (
            mock_cls.call_args.kwargs if mock_cls.called else mock_cls.call_args_list[-1].kwargs
        )
