...
_CONVERSION_SRC = (
    Path(__file__).resolve().parents[4] / "src/telaios/tools/builtin/documents/conversion.py"
)
...
class TestConvertToMarkdown:
    ...
    async def test_html_strips_script_and_style(self):
        ...
        html = (
            b"<html><body><script>var x=1;</script><style>.a{}</style><p>Content</p></body></html>"
        )
        result = await convert_to_markdown(html, "text/html")
