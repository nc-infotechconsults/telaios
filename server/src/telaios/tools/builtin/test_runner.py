...
_KNOWN_FRAMEWORKS = [
    TestFramework(
        "vitest", "npx vitest run --reporter verbose 2>&1", r"(\d+)\s+passed", r"(\d+)\s+failed"
    ),
    TestFramework(
        "jest",
...
async def detect_framework(workspace_path: str) -> TestFramework | None:
    pkg_json = os.path.join(workspace_path, "package.json")
    if os.path.isfile(pkg_json):  # noqa: ASYNC240
        try:
            with open(pkg_json, encoding="utf-8") as fh:  # noqa: ASYNC230
                pkg = json.load(fh)
            all_deps = {**pkg.get("devDependencies", {}), **pkg.get("dependencies", {})}
            scripts = pkg.get("scripts", {})
            if "vitest" in all_deps or "vitest" in scripts.get("test", ""):
                return next(
                    framework for framework in _KNOWN_FRAMEWORKS if framework.name == "vitest"
                )
            if "jest" in all_deps or "jest" in scripts.get("test", ""):
                return next(
                    framework for framework in _KNOWN_FRAMEWORKS if framework.name == "jest"
                )
            if "mocha" in all_deps or "mocha" in scripts.get("test", ""):
                return next(
                    framework for framework in _KNOWN_FRAMEWORKS if framework.name == "mocha"
                )
        except Exception:
            pass
    ...
    for indicator, name in [
        ...
        if not os.path.isfile(indicator_path):  # noqa: ASYNC240
            continue
        if name == "pytest":
            try:
                with open(indicator_path) as fh:  # noqa: ASYNC230
                    if "pytest" in fh.read() or indicator == "pytest.ini":
                        return next(
                            framework for framework in _KNOWN_FRAMEWORKS if framework.name == name
                        )
                continue
            except Exception:
                pass
        return next(framework for framework in _KNOWN_FRAMEWORKS if framework.name == name)
...
async def run_tests(
    ...
    passed = (
        ...
        if pass_match and pass_match.lastindex
        else (1 if exit_code == 0 else 0)
    )
    failed = (
        int(fail_match.group(1))
        if fail_match and fail_match.lastindex
        else (1 if exit_code != 0 else 0)
    )
