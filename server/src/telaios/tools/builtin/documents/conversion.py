...
def _format_docx_paragraph(para: Any) -> str:
...
def _html_to_markdown(buffer: bytes) -> str:
    ...
    try:
        ...
        for tag in soup(["script", "style", "nav", "header", "footer"]):
            tag.decompose()
        return cast(str, markdownify.markdownify(str(soup), heading_style="ATX"))
    except ImportError:
...
<body>{html_body}</body>
        ...
        raise ValueError("PDF conversion requires weasyprint: pip install weasyprint") from None
    except Exception as exc:
