...
def build_coding_tools(ctx: dict[str, Any]) -> list[Any]:
...
def build_review_tools(ctx: dict[str, Any]) -> list[Any]:
    repo = ctx.get("repo") or ctx.get("workspace_path") or "."
    run_shell = make_run_shell_tool(repo, allowed_prefixes=["git", "cat", "ls", "pwd"])
    finish = make_finish_tool(
        {"approved": "boolean", "summary": "string", "required_changes": "array"}
    )

    async def _finish_review(
        approved: bool, summary: str, required_changes: list[Any] | None = None
    ) -> str:
        return json.dumps(
            {
                "approved": approved,
...
def build_testing_tools(ctx: dict[str, Any]) -> list[Any]:
    repo = ctx.get("repo") or ctx.get("workspace_path") or "."
    finish = make_finish_tool(
        {"passed": "boolean", "summary": "string", "tests_run": "integer", "failures": "array"}
    )
