...
def make_run_shell_tool(
    ...
    async def _run_shell(command: str, **_: Any) -> str:
        ...
        if allowed_prefixes is not None and tokens[0] not in allowed_prefixes:
            return f"Error: command '{tokens[0]}' is not in the allowed list: {allowed_prefixes}"

        workspace = Path(workspace_path).resolve()  # noqa: ASYNC240
        try:
            ...
            try:
                stdout, _ = await asyncio.wait_for(
                    proc.communicate(),  # type: ignore[arg-type]
                    timeout=timeout,
                )
            except TimeoutError:
                proc.kill()
