...
def _json_schema_to_parameter(schema: dict[str, Any]) -> ToolParameter:
    ...
    param_type: Literal["string", "number", "integer", "boolean", "array", "object"] = (
    ...
    if param_type == "object" and "properties" in schema:
        properties = {k: _json_schema_to_parameter(v) for k, v in schema["properties"].items()}
...
def mcp_schema_to_input_schema(input_schema: dict[str, Any]) -> ToolInputSchema:
    ...
    properties: dict[str, ToolParameter] = {
        name: _json_schema_to_parameter(prop_schema) for name, prop_schema in raw_properties.items()
    }
