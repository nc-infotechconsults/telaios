...
from telaios.core.reranker import Reranker
from telaios.core.types import RerankerConfig, RetrievalQuery, RetrievalResult
...
def _create_reranker_from_config(config: RerankerConfig) -> Reranker:
    ...
    if provider == "voyage":
        ...
        return VoyageReranker(
            ...
            model=config.model or "rerank-2",
            config=config,
        )

    raise ValueError(
        f"Unknown reranker provider: {config.provider!r}. Supported: 'cross_encoder', 'voyage'."
    )
