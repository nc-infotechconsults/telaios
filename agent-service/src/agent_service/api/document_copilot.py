"""
Document Copilot API — AI-powered document operations.

Endpoints:
  POST /projects/{project_id}/documents/{document_id}/copilot/summarize
  POST /projects/{project_id}/documents/{document_id}/copilot/ask
  POST /projects/{project_id}/documents/{document_id}/copilot/extract

These endpoints are called directly by the data-api (acting as a proxy) or
by the frontend via an internal forwarded route.

LLM credentials are sourced from the global Settings (LLM_PROVIDER, LLM_MODEL,
LLM_API_KEY, LLM_BASE_URL) so no per-request credential is needed.
"""
