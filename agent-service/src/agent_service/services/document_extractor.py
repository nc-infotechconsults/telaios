...
------------------------------------------------
Extract plain text from a file buffer using multiple format handlers.

Supports: PDF, DOCX, XLSX, PPTX, HTML, Markdown, EML, MSG, plain text.
...
Sources
~~~~~~~
- PyMuPDF (PDF): https://pymupdf.readthedocs.io/
...
async def extract_text(
    ...
    if mime == "application/octet-stream" and ext:
        text_exts = {
            ...
            "toml", "xml", "html", "css", "sh", "java", "c", "cpp", "h",
        }
        if ext in text_exts:
            return buffer.decode("utf-8", errors="replace")
...
def _extract_pdf(buffer: bytes) -> str:
    ...
    Process:
        1. Extract text using PyMuPDF (fast, handles native text)
        2. If page has very little text (< 50 chars), try OCR on images
        3. Add page markers for PageChunker compatibility

    Requires ``pytesseract`` and ``pillow`` for OCR fallback.
    """
    try:
        import fitz  # PyMuPDF
        ...
        for page_num, page in enumerate(doc, start=1):
            ...
            # Fallback to OCR if page has very little text (likely scanned/image PDF)
            ...
            # Add page marker for PageChunker
...
def _extract_pdf_page_ocr(page) -> str:
    ...
    try:
        ...
        # Render page to image at 200 DPI (good balance for OCR)
...
def _extract_pptx(buffer: bytes) -> str:
    ...
    Extracts from slide titles, body text, and speaker notes.
...
def _extract_eml(buffer: bytes) -> str:
    ...
    try:
        ...
        msg = Parser().parsebytes(buffer, policy=policy.default)
...
def _extract_msg(buffer: bytes) -> str:
    ...
    Source: https://github.com/TeamMsgExtractor/msg-extractor
    """
    try:
        import extract_msg
