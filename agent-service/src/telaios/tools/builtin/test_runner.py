    ...
    if os.path.isfile(pkg_json):
        try:
            ...
            if "vitest" in all_deps or "vitest" in scripts.get("test", ""):
                return next(framework for framework in _KNOWN_FRAMEWORKS if framework.name == "vitest")
            if "jest" in all_deps or "jest" in scripts.get("test", ""):
                return next(framework for framework in _KNOWN_FRAMEWORKS if framework.name == "jest")
            if "mocha" in all_deps or "mocha" in scripts.get("test", ""):
                return next(framework for framework in _KNOWN_FRAMEWORKS if framework.name == "mocha")
        except Exception:
            pass
