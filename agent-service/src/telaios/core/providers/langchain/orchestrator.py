...
from telaios.core.types import AgentConfig, AgentInput, MessageRole, Message
...
class LangChainOrchestrator(LangChainAgent, Orchestrator):
    ...
    def _build_sub_agent_tool(
        self, name: str, description: str, agent: Agent
    ) -> "StructuredTool":
        """
        ...
        async def _invoke(task_description: str) -> str:
            try:
                result = await agent.run(
                    AgentInput(
                        messages=[
                            Message(role=MessageRole.HUMAN, content=task_description)
                        ]
                    )
                )
