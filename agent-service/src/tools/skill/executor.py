...
from tools.skill.types import SkillManifest, SkillScript
    ...
    async def execute(
        ...
        if self.allowed_commands:
            ...
            if command_name not in self.allowed_commands:
                # Check if it's a bash script (allowed by default)
                if not command_name.endswith(".sh"):
                    raise PermissionError(
                        f"Script '{command_name}' not in allowed commands list"
                    )
