...
from core.types import RetrievalQuery, RetrievalResult
...
def _create_reranker_from_config(config: RerankerConfig) -> Reranker:
    ...
    if provider == "cross_encoder" or provider == "cross-encoder":
        from core.providers.cross_encoder.reranker import CrossEncoderReranker

        return CrossEncoderReranker(config=config)

    if provider == "voyage":
        from core.providers.voyage.reranker import VoyageReranker
